...
## 前言
- 我们尊重每一份乐于分享的热忱。管理员们在精力允许的情况下会尽量耐心地说明和指导，希望你耐心仔细地阅读并遵守规则
- 规则对于资源有较为严格的要求，是因为网站建立的目的并非临时交流和传输资源，而是希望每一个合规的资源在发布后能永远存在并可供下载，为将来的用户持续提供价值和方便，而非造成迷惑、混乱和不便
...
- 我们非常欢迎自购、自抓、自压等原创资源，符合规则且质量过关的此类资源可以获得 72 小时“免费 & 2X 上传”，你可以在副标题中指出，或直接使用“举报”功能来告知管理组以获得促销
...
- 标题
- 外部链接
    - 请务必填上正确的 IMDb 链接（如果有的话），这有助于更准确地搜索某部影片、查询该影片的其他版本和我们对资源的管理
    - 请务必填上正确的豆瓣链接（如果有的话），某些条目可能需要登录豆瓣账号才能搜索到结果
- NFO 文件
- 简介
    - 请务必在简介中附上影片的 MediaInfo（[获取方法](http://www.nexushd.org/forums.php?action=viewtopic&forumid=51&topicid=19594)），这能非常直观地展示影片的视频、音频、字幕等轨道的信息
- 类型
- 质量
...
- 共存
    ...
    - 对于视频媒介，HD 蓝光原盘、UHD 蓝光原盘、HD Remux、UHD Remux、HD DVD 原盘、DVD 原盘各保留一个版本；影片不发售碟片时，保留一个版本的 HDTV 流媒体或 WEB-DL/WEBRip 流媒体 ***// 新增 UHD、WEBRip***
...
- 淘汰（Trump）
    - Trump 达成后，旧版本会被删除，新版本获得 24 小时“免费”
    - 对于高清重编码（HD）
        - 在 BDRip 发布后，所有在来源质量上不如 BDRip 的版本（HDTV、WEB-DL 等）都会被立刻删除（最终判断依据始终为来源的实际质量和观看体验，例如，如果 Blu-ray Source 较 WEB Source 质量更差，则来自 WEB Source 的优质重编码将被优先考虑保留），此条不达成 Trump，
        - 新发布的 BDRip 必须比已发布的 BDRip 拥有更好的视频质量，否则会被立刻删除
        - 允许一段时间内多个版本 BDRip 共存，管理组会选取一个（或最多不超过两个）相对较优版本并给予 24 小时“免费” ***// 最终版本给 72 h 置顶 free 和[推荐]标签 这条好像没有执行过？***
...
- 附加信息        
    - PROPER 版本发布时，原先的版本视为 Nuked（劣质），REPACK/RERIP 版本发布时，同一发布组或个人发布的原先版本视为 Nuked（劣质），新版本获得 24 小时“免费” ***// 细化***
    - 质量判断依据
