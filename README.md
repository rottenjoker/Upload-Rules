...
- 随机促销（种子上传后系统自动随机设为促销）
    - 5% 的概率成为“免费”，自发布起限时 7 天
    - 3% 的概率成为“免费 & 2x 上传”，自发布起限时 3 天 ***// 根据实际观察***
- 总体积大于 20GB 的种子将自动成为“50% 下载”
- Blu-ray，HD DVD 原盘，Remux 将成为“50% 下载” ***// 新增 remux***
- UHD Blu-ray 原盘，UHD Remux 将成为“30% 下载” ***// 新增 remux***
- 关注度高的种子将被设置为促销（由管理员定夺）
- 促销变化
    - ~~所有的随机促销限时 7 天（自种子发布时起）~~
    - 非特殊类种子在发布 1 个月后将自动永久成为“2x 上传”
    - 特殊类种子（包括但不限于上述“50% 下载”的几种情况）在发布 1 个月后将自动永久成为“50% 下载 & 2x 上传”
- 符合规则且质量过关的自购、自抓、自压等原创资源可以获得 72 小时“免费 & 2x 上传” ***// 新增***
    - 你可以在副标题中标注，或直接使用“举报”功能来告知管理组以获得促销
...
- 通过各种录屏方式获得的偷跑（Pre-Retail）资源，以及属于高清或标清级别但质量较差的视频文件，包括但不限于 CAM、TC、TS (TELESYNC)、SCR、DVDSCR、R5、R5.Line、HalfCD、HDRip、韩国 HC（硬字幕）等 ***// 整合，HDRip？***
- RealVideo 编码的视频（通常封装于 RMVB 或 RM）、flv 文件
- 单独的样片（样片请和正片一起上传）
- RAR 等压缩文件
- 重复（Dupe）的资源（详见此处）
- 涉及禁忌或敏感内容（如色情、暴力、宗教、敏感政治话题等）的资源（尺度由管理组决定） ***// 整合***
- 损坏的文件，指在读取或回放过程中出现错误的文件
- 垃圾文件，如病毒、木马、网站链接、广告文档、种子中包含的种子文件等，或无关文件
- 未使用无损来源或经过多次重编码的资源，例如 BRRip ***// 新增***
- [未经候选发布的电影合集](http://www.nexushd.org/forums.php?action=viewtopic&topicid=20573&page=p850734#pid850734)（资源打包规则详见此处）***// 整合***
- 一些特定资源 ***// 整合***
...
## WEB-DL 和 WEBRip 资源的规则增补 ***// 新增***
- WEB-DL
    - 除下文 WEBRip 提到的情况以外，不允许来源于此的重编码资源
- WEBRip
    - 允许发布的情况仅限于
        - 相同分辨率下，无法获得 WEB-DL 资源（此时视其与 WEB-DL 同级）
        - 更高分辨率的来源有更优质的视频质量，例如 720p WEBRip from 1080p/4K Source, 1080p WEBRip from 4K source
...
- 标题
    - 基本格式
        - 英文片名 上映年份 [剪辑版本] [发布说明] 分辨率标准 来源介质 [音频编码格式] 视频编码格式-发布组
    - 若种子文件命名大致符合上述格式，则更推荐保留
    - 音频、视频编码格式的先后顺序并非强制
    - 建议参考较新的范例
- 外部链接（如果有的话）
    - 请务必填上正确的 IMDb 链接
...
- 共存
    ...
    - 对于高清重编码（HD）
        - 1080p Rip 和 720p Rip 原则上各保留一个版本，最多不超过两个 ***// 新增“最多……”***
        - 对于来源为 UHD（2160p）HDR 的资源，原则上只保留一个版本的 1080p Rip ***// 新增***
    - 对于超高清重编码（UHD） ***// 新增***
        - 原则上只保留一个版本的 2160p Rip
    - 对于视频媒介，HD & UHD 蓝光原盘、HD & UHD Remux、HD DVD 原盘、DVD 原盘各保留一个版本；影片不发售碟片时，保留一个版本的 HDTV 流媒体或 WEB-DL 流媒体 ***// 新增 UHD***
...
- 淘汰（Trump）
    - 对于高清重编码（HD）
        ...
        - 相同分辨率下，新发布的 BDRip 必须拥有和已发布版本相近的，或更好的视频质量，否则会被立刻删除 ***// 调整***
            - 质量更好的版本发布后，旧版本将被删除，新版本获得 24 小时“免费” ***// 主要针对常规洗版***
    ...
    - 其他情况
        ...
        - 对于 WEB-DL（例如 Amazon/Netflix 4k SDR WEB-DL）或 HD DVD 优于 Blu-ray 的情况，管理组会根据来源和重编码质量优劣进行类似的版本选取工作 ***// 新增***
- 对于断种
